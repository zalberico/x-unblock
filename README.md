...
I thought I might be able to work around this by using the API to unblock, but the backend call also fails to return the blocked users. I shared the code anyway in case it's useful - even if it did work as it's supposed to the severe rate limits would make it very slow to complete.
